...
import anec.ml.yarn.anecyarn.common.JobPriority;
...
class ClientArguments {
  ...
  Properties inputs;
  Properties outputs;
  String anecyarnCacheFiles;
  String anecyarnCacheArchives;
  int priority;
  String queue;
  String userPath;
  ...
  private void init() {
    appName = "";
    appType = AnecYarnConfiguration.DEFAULT_ANECYARN_APP_TYPE.toUpperCase();
    amMem = AnecYarnConfiguration.DEFAULT_ANECYARN_AM_MEMORY;
    amCores = AnecYarnConfiguration.DEFAULT_ANECYARN_AM_CORES;
    workerMemory = AnecYarnConfiguration.DEFAULT_ANECYARN_WORKER_MEMORY;
    workerVCores = AnecYarnConfiguration.DEFAULT_ANECYARN_WORKER_VCORES;
    workerNum = AnecYarnConfiguration.DEFAULT_ANECYARN_WORKER_NUM;
    psMemory = AnecYarnConfiguration.DEFAULT_ANECYARN_PS_MEMORY;
    psVCores = AnecYarnConfiguration.DEFAULT_ANECYARN_PS_VCORES;
    psNum = AnecYarnConfiguration.DEFAULT_ANECYARN_PS_NUM;
    anecyarnFiles = null;
    libJars = null;
    launchCmd = "";
    anecyarnCacheFiles = "";
    anecyarnCacheArchives = "";
    appMasterJar = "";
    userPath = "";
    userPath = "";
    priority = AnecYarnConfiguration.DEFAULT_ANECYARN_APP_PRIORITY;
    queue = "";
    userClasspathFirst = AnecYarnConfiguration.DEFAULT_ANECYARN_USER_CLASSPATH_FIRST;
    boardIndex = AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_WORKER_INDEX;
    boardReloadInterval = AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_RELOAD_INTERVAL;
    boardEnable = AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_ENABLE;
    boardLogDir = AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_LOG_DIR;
    boardHistoryDir = AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_HISTORY_DIR;
    boardModelPB = AnecYarnConfiguration.DEFAULT_ANECYARN_BOARD_MODELPB;
    boardCacheTimeout = AnecYarnConfiguration.DEFAULT_ANECYARN_BOARD_CACHE_TIMEOUT;
    isRenameInputFile = AnecYarnConfiguration.DEFAULT_ANECYARN_INPUTFILE_RENAME;
    streamEpoch = AnecYarnConfiguration.DEFAULT_ANECYARN_STREAM_EPOCH;
    inputStreamShuffle = AnecYarnConfiguration.DEFAULT_ANECYARN_INPUT_STREAM_SHUFFLE;
    inputFormatClass = AnecYarnConfiguration.DEFAULT_ANECYARN_INPUTF0RMAT_CLASS;
    outputFormatClass = AnecYarnConfiguration.DEFAULT_ANECYARN_OUTPUTF0RMAT_CLASS;
    inputStrategy = AnecYarnConfiguration.DEFAULT_ANECYARN_INPUT_STRATEGY.toUpperCase();
    outputStrategy = AnecYarnConfiguration.DEFAULT_ANECYARN_OUTPUT_STRATEGY.toUpperCase();
    tfEvaluator = AnecYarnConfiguration.DEFAULT_ANECYARN_TF_EVALUATOR;
    outputIndex = -1;
    ...
    allOptions.addOption("appType", "app-type", true,
        "set the Application type, default \"ANECYARN\"");
    ...
    allOptions.addOption("files", "files", true,
        "Location of the AnecYarn files used in container");
    allOptions.addOption("jars", "jars", true,
        "Location of the AnecYarn lib jars used in container");

    allOptions.addOption("launchCmd", "launch-cmd", true, "Cmd for AnecYarn program");
    ...
    allOptions.addOption("cacheFile", "cacheFile", true,
        "add the AnecYarn hdfsFile PATH");
    allOptions.addOption("cacheArchive", "cacheArchive", true,
        "add the AnecYarn hdfsPackage PATH");
    ...
    OptionBuilder
        .withValueSeparator('=');
    OptionBuilder
        .withDescription("AnecYarn configure");
    ...
    OptionBuilder
        .withValueSeparator('#');
    OptionBuilder
        .withDescription("dfs location,representing the source data of AnecYarn");
    ...
    OptionBuilder
        .withValueSeparator('#');
    OptionBuilder
        .withDescription("dfs location,representing the AnecYarn result");
  ...
  private void cliParser(String[] args) throws ParseException, IOException, ClassNotFoundException {
    ...
    if (appName.trim().equals("")) {
      appName = "AnecYarn-" + System.currentTimeMillis();
    }
    ...
    if (cliParser.hasOption("conf")) {
      ...
      if (!"TENSORFLOW".equals(appType) && !"MXNET".equals(appType) && !appType.equals("LIGHTLDA") && !appType.equals("XFLOW")) {
        if (confs.containsKey("anecyarn.ps.num")) {
          confs.setProperty("anecyarn.ps.num", "0");
        }
      }
    }
    ...
    if (cliParser.hasOption("cacheFile")) {
      anecyarnCacheFiles = cliParser.getOptionValue("cacheFile");
    }

    if (cliParser.hasOption("cacheArchive")) {
      anecyarnCacheArchives = cliParser.getOptionValue("cacheArchive");
    }

    if (cliParser.hasOption("files")) {
      anecyarnFiles = StringUtils.split(cliParser.getOptionValue("files"), ",");
    }
