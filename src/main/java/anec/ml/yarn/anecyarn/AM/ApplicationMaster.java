...
public class ApplicationMaster extends CompositeService {
  ...
  private String anecyarnCommand;
  ...
  private ConcurrentHashMap<AnecYarnContainerId, List<InputInfo>> containerId2InputInfo;
  private InputSplit[] inputFileSplits;
  private ConcurrentHashMap<AnecYarnContainerId, List<InputSplit>> containerId2InputSplit;
  ...
  private ApplicationMaster() {
    ...
    conf = new AnecYarnConfiguration();
    conf.addResource(new Path(AnecYarnConstants.ANECYARN_JOB_CONFIGURATION));
    System.setProperty(AnecYarnConstants.Environment.HADOOP_USER_NAME.toString(), conf.get("hadoop.job.ugi").split(",")[0]);
    ...
    statusUpdateInterval = conf.getInt(AnecYarnConfiguration.ANECYARN_STATUS_UPDATE_INTERVAL, AnecYarnConfiguration.DEFAULT_ANECYARN_STATUS_PULL_INTERVAL);
    applicationAttemptID = Records.newRecord(ApplicationAttemptId.class);
    applicationMessageQueue = new LinkedBlockingQueue<>(
        conf.getInt(AnecYarnConfiguration.ANECYARN_MESSAGES_LEN_MAX, AnecYarnConfiguration.DEFAULT_ANECYARN_MESSAGES_LEN_MAX));
    containerLocalResource = new HashMap<>();
    applicationContext = new RunningAppContext();

    envs = System.getenv();
    workerMemory = conf.getInt(AnecYarnConfiguration.ANECYARN_WORKER_MEMORY, AnecYarnConfiguration.DEFAULT_ANECYARN_WORKER_MEMORY);
    workerVCores = conf.getInt(AnecYarnConfiguration.ANECYARN_WORKER_VCORES, AnecYarnConfiguration.DEFAULT_ANECYARN_WORKER_VCORES);
    workerNum = conf.getInt(AnecYarnConfiguration.ANECYARN_WORKER_NUM, AnecYarnConfiguration.DEFAULT_ANECYARN_WORKER_NUM);
    psMemory = conf.getInt(AnecYarnConfiguration.ANECYARN_PS_MEMORY, AnecYarnConfiguration.DEFAULT_ANECYARN_PS_MEMORY);
    psVCores = conf.getInt(AnecYarnConfiguration.ANECYARN_PS_VCORES, AnecYarnConfiguration.DEFAULT_ANECYARN_PS_VCORES);
    psNum = conf.getInt(AnecYarnConfiguration.ANECYARN_PS_NUM, AnecYarnConfiguration.DEFAULT_ANECYARN_PS_NUM);
    single = conf.getBoolean(AnecYarnConfiguration.ANECYARN_MODE_SINGLE, AnecYarnConfiguration.DEFAULT_ANECYARN_MODE_SINGLE);
    appPriority = conf.getInt(AnecYarnConfiguration.ANECYARN_APP_PRIORITY, AnecYarnConfiguration.DEFAULT_ANECYARN_APP_PRIORITY);
    ...
    hostLocals = null;
    tfEvaluator = conf.getBoolean(AnecYarnConfiguration.ANECYARN_TF_EVALUATOR, AnecYarnConfiguration.DEFAULT_ANECYARN_TF_EVALUATOR);
    ...
    if (applicationAttemptID.getAttemptId() > 1 && (conf.getInt(AnecYarnConfiguration.ANECYARN_APP_MAX_ATTEMPTS, AnecYarnConfiguration.DEFAULT_ANECYARN_APP_MAX_ATTEMPTS) > 1)) {
      int maxMem = Integer.valueOf(envs.get(AnecYarnConstants.Environment.ANECYARN_CONTAINER_MAX_MEMORY.toString()));
      LOG.info("maxMem : " + maxMem);
      workerMemory = workerMemory + (applicationAttemptID.getAttemptId() - 1) * (int) Math.ceil(workerMemory * conf.getDouble(AnecYarnConfiguration.ANECYARN_WORKER_MEM_AUTO_SCALE, AnecYarnConfiguration.DEFAULT_ANECYARN_WORKER_MEM_AUTO_SCALE));
      LOG.info("Auto Scale the Worker Memory from " + conf.getInt(AnecYarnConfiguration.ANECYARN_WORKER_MEMORY, AnecYarnConfiguration.DEFAULT_ANECYARN_WORKER_MEMORY) + " to " + workerMemory);
      if (workerMemory > maxMem) {
        workerMemory = maxMem;
      }
      if (psNum > 0) {
        psMemory = psMemory + (applicationAttemptID.getAttemptId() - 1) * (int) Math.ceil(psMemory * conf.getDouble(AnecYarnConfiguration.ANECYARN_PS_MEM_AUTO_SCALE, AnecYarnConfiguration.DEFAULT_ANECYARN_PS_MEM_AUTO_SCALE));
        LOG.info("Auto Scale the Ps Memory from " + conf.getInt(AnecYarnConfiguration.ANECYARN_PS_MEMORY, AnecYarnConfiguration.DEFAULT_ANECYARN_PS_MEMORY) + " to " + psMemory);
        if (psMemory > maxMem) {
          psMemory = maxMem;
        }
      }
    ...
    if (envs.containsKey(AnecYarnConstants.Environment.ANECYARN_FILES_LOCATION.toString())) {
      appFilesRemoteLocation = envs.get(AnecYarnConstants.Environment.ANECYARN_FILES_LOCATION.toString());
      LOG.info("Application files location: " + appFilesRemoteLocation);
    }

    if (envs.containsKey(AnecYarnConstants.Environment.ANECYARN_LIBJARS_LOCATION.toString())) {
      appLibJarsRemoteLocation = envs.get(AnecYarnConstants.Environment.ANECYARN_LIBJARS_LOCATION.toString());
      LOG.info("Application lib Jars location: " + appLibJarsRemoteLocation);
    }

    if (envs.containsKey(AnecYarnConstants.Environment.ANECYARN_CACHE_FILE_LOCATION.toString())) {
      appCacheFilesRemoteLocation = envs.get(AnecYarnConstants.Environment.ANECYARN_CACHE_FILE_LOCATION.toString());
      LOG.info("Application cacheFiles location: " + appCacheFilesRemoteLocation);
    }

    if (envs.containsKey(AnecYarnConstants.Environment.ANECYARN_CACHE_ARCHIVE_LOCATION.toString())) {
      appCacheArchivesRemoteLocation = envs.get(AnecYarnConstants.Environment.ANECYARN_CACHE_ARCHIVE_LOCATION.toString());
      LOG.info("Application cacheArchive location: " + appCacheArchivesRemoteLocation);
    }

    assert (envs.containsKey(AnecYarnConstants.Environment.APP_JAR_LOCATION.toString()));
    appJarRemoteLocation = new Path(envs.get(AnecYarnConstants.Environment.APP_JAR_LOCATION.toString()));
    LOG.info("Application jar location: " + appJarRemoteLocation);

    assert (envs.containsKey(AnecYarnConstants.Environment.ANECYARN_JOB_CONF_LOCATION.toString()));
    appConfRemoteLocation = new Path(envs.get(AnecYarnConstants.Environment.ANECYARN_JOB_CONF_LOCATION.toString()));
    LOG.info("Application conf location: " + appConfRemoteLocation);

    if (envs.containsKey(AnecYarnConstants.Environment.ANECYARN_EXEC_CMD.toString())) {
      anecyarnCommand = envs.get(AnecYarnConstants.Environment.ANECYARN_EXEC_CMD.toString());
      LOG.info("AnecYarn exec command: " + anecyarnCommand);
    }

    if (envs.containsKey(AnecYarnConstants.Environment.ANECYARN_APP_TYPE.toString())) {
      anecyarnAppType = envs.get(AnecYarnConstants.Environment.ANECYARN_APP_TYPE.toString()).toUpperCase();
      LOG.info("AnecYarn app type: " + anecyarnAppType);
    } else {
      anecyarnAppType = AnecYarnConfiguration.DEFAULT_ANECYARN_APP_TYPE.toUpperCase();
      LOG.info("AnecYarn app type: " + anecyarnAppType);
    }
  ...
  private void init() {
    ...
    applicationHistoryUrl = conf.get(AnecYarnConfiguration.ANECYARN_HISTORY_WEBAPP_ADDRESS,
        AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_WEBAPP_ADDRESS) + "/jobhistory/job/"
    ...
    cleanApplication = new Thread(new Runnable() {
      @Override
      public void run() {
        System.clearProperty(AnecYarnConstants.Environment.HADOOP_USER_NAME.toString());
        YarnConfiguration anecyarnConf = new YarnConfiguration();
        if (anecyarnConf.getBoolean(AnecYarnConfiguration.ANECYARN_CLEANUP_ENABLE, AnecYarnConfiguration.DEFAULT_ANECYARN_CLEANUP_ENABLE)) {
          Path stagingDir = new Path(envs.get(AnecYarnConstants.Environment.ANECYARN_STAGING_LOCATION.toString()));
          try {
            stagingDir.getFileSystem(anecyarnConf).delete(stagingDir);
        ...
        try {
          ...
          Path logdir = new Path(conf.get(AnecYarnConfiguration.ANECYARN_HISTORY_LOG_DIR,
              AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_LOG_DIR) + "/" + applicationAttemptID.getApplicationId().toString()
              + "/" + applicationAttemptID.getApplicationId().toString());
          Path jobLogPath = new Path(anecyarnConf.get("fs.defaultFS"), logdir);
          LOG.info("jobLogPath:" + jobLogPath.toString());
          LOG.info("Start write the log to " + jobLogPath.toString());
          FileSystem fs = FileSystem.get(anecyarnConf);
          FSDataOutputStream out = fs.create(jobLogPath);
          fs.setPermission(jobLogPath, new FsPermission(LOG_FILE_PERMISSION));
          if (conf.getBoolean(AnecYarnConfiguration.ANECYARN_HOST_LOCAL_ENABLE, AnecYarnConfiguration.DEFAULT_ANECYARN_HOST_LOCAL_ENABLE)) {
            Path hostLocaldir = new Path(conf.get(AnecYarnConfiguration.ANECYARN_HISTORY_LOG_DIR,
                AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_LOG_DIR) + "/" + conf.get("hadoop.job.ugi").split(",")[0]
                + "/" + envs.get(AnecYarnConstants.Environment.ANECYARN_APP_NAME.toString()));
            Path hostLocalPath = new Path(anecyarnConf.get("fs.defaultFS"), hostLocaldir);
            try {
          ...
          logMessage.put(AMParams.APP_TYPE, anecyarnAppType);

          String tensorboardInfo = "-";
          if (conf.getBoolean(AnecYarnConfiguration.ANECYARN_TF_BOARD_ENABLE, AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_ENABLE)) {
            Path boardLogPath;
            if (conf.get(AnecYarnConfiguration.ANECYARN_TF_BOARD_LOG_DIR, AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_LOG_DIR).indexOf("hdfs://") == -1) {
              if (conf.get(AnecYarnConfiguration.ANECYARN_TF_BOARD_HISTORY_DIR, AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_HISTORY_DIR).equals(anecyarnConf.get(AnecYarnConfiguration.ANECYARN_TF_BOARD_HISTORY_DIR, AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_HISTORY_DIR))) {
                boardLogPath = new Path(anecyarnConf.get("fs.defaultFS"), conf.get(AnecYarnConfiguration.ANECYARN_TF_BOARD_HISTORY_DIR,
                    AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_HISTORY_DIR) + "/" + applicationAttemptID.getApplicationId().toString());
              } else {
                boardLogPath = new Path(conf.get("fs.defaultFS"), conf.get(AnecYarnConfiguration.ANECYARN_TF_BOARD_HISTORY_DIR,
                    AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_HISTORY_DIR));
              }
            } else {
              boardLogPath = new Path(conf.get(AnecYarnConfiguration.ANECYARN_TF_BOARD_LOG_DIR));
            }
          ...
          Map<AnecYarnContainerId, String> reporterProgress = applicationContext.getReporterProgress();
          Map<AnecYarnContainerId, String> containersAppStartTime = applicationContext.getContainersAppStartTime();
          Map<AnecYarnContainerId, String> containersAppFinishTime = applicationContext.getContainersAppFinishTime();
          for (Container container : workerContainers) {
            ...
            if (tfEvaluator && container.getId().toString().equals(tfEvaluatorContainerId)) {
              containerMessage.put(AMParams.CONTAINER_ROLE, AnecYarnConstants.EVALUATOR);
            } else {
              containerMessage.put(AMParams.CONTAINER_ROLE, AnecYarnConstants.WORKER);
            }
            if (applicationContext.getContainerStatus(new AnecYarnContainerId(container.getId())) != null) {
              containerMessage.put(AMParams.CONTAINER_STATUS, applicationContext.getContainerStatus(new AnecYarnContainerId(container.getId())).toString());
            } else {
            ...
            if (applicationContext.getContainersCpuMetrics().get(new AnecYarnContainerId(container.getId())) != null) {
              ConcurrentHashMap<String, LinkedBlockingDeque<Object>> cpuMetrics = applicationContext.getContainersCpuMetrics().get(new AnecYarnContainerId(container.getId()));
              containerMessage.put(AMParams.CONTAINER_CPU_METRICS, new Gson().toJson(cpuMetrics));
            }

            if (applicationContext.getContainersCpuStatistics().get(new AnecYarnContainerId(container.getId())) != null) {
              ConcurrentHashMap<String, List<Double>> cpuStatistics = applicationContext.getContainersCpuStatistics().get(new AnecYarnContainerId(container.getId()));
              ...
              if (cpuStatistics.size() != 0) {
                ...
                if (cpuMemUsagedMax * 1024.0 / workerMemory < conf.getDouble(AnecYarnConfiguration.ANECYARN_CONTAINER_MEM_USAGE_WARN_FRACTION, AnecYarnConfiguration.DEFAULT_ANECYARN_CONTAINER_MEM_USAGE_WARN_FRACTION)) {
  ...
  private void buildInputFileStatus() {
    String anecyarnInputs = envs.get(AnecYarnConstants.Environment.ANECYARN_INPUTS.toString());
    if (StringUtils.isBlank(anecyarnInputs)) {
      LOG.info("Application has no inputs");
      return;
    }

    String[] inputs = StringUtils.split(anecyarnInputs, "|");
    if (inputs != null && inputs.length > 0) {
      for (String input : inputs) {
        String[] inputPathTuple = StringUtils.split(input, "#");
        if (inputPathTuple.length < 2) {
          throw new RuntimeException("Error input path format " + anecyarnInputs);
        }
  ...
  public void buildInputStreamFileStatus() throws IOException {
    ...
    String[] inputPathTuple = StringUtils.split(anecyarnInputs, "#");
    if (inputPathTuple.length < 2) {
      throw new RuntimeException("Error input path format " + anecyarnInputs);
    }
    String inputPathRemote = inputPathTuple[0];
    if (!StringUtils.isBlank(inputPathRemote)) {
      ...
      jobConf.set(AnecYarnConstants.STREAM_INPUT_DIR, inputPathRemote);
  ...
  private void allocateInputSplits() {
    ...
    for (Container container : acquiredWorkerContainers) {
      ...
      containerId2InputInfo.putIfAbsent(new AnecYarnContainerId(container.getId()), new ArrayList<InputInfo>());
    }
    ...
    for (String fileName : fileKeys) {
      ...
      ConcurrentHashMap<AnecYarnContainerId, ConcurrentHashMap<String, InputInfo>> containersFiles = new ConcurrentHashMap<>();
      for (int i = 0, len = paths.size(); i < len; i++) {
        Integer index = i % splitWorkerNum;
        ConcurrentHashMap<String, InputInfo> mapSplit;
        AnecYarnContainerId containerId = new AnecYarnContainerId(acquiredWorkerContainers.get(index).getId());
      ...
      Set<AnecYarnContainerId> containerIdSet = containersFiles.keySet();
      for (AnecYarnContainerId containerId : containerIdSet) {
  ...
  private void allocateInputStreamSplits() {
    ...
    for (Container container : acquiredWorkerContainers) {
      LOG.info("Initializing " + container.getId().toString() + " input splits");
      containerId2InputSplit.putIfAbsent(new AnecYarnContainerId(container.getId()), new ArrayList<InputSplit>());
    }
    int splitWorkerNum = workerNum;
    if (tfEvaluator) {
      splitWorkerNum--;
    ...
    if (conf.getBoolean(AnecYarnConfiguration.ANECYARN_INPUT_STREAM_SHUFFLE, AnecYarnConfiguration.DEFAULT_ANECYARN_INPUT_STREAM_SHUFFLE)) {
      LOG.info("ANECYARN_INPUT_STREAM_SHUFFLE is true");
      ...
      LOG.info("ANECYARN_INPUT_STREAM_SHUFFLE is false");
      ...
      for (int i = 0; i < splitWorkerNum; i++) {
        AnecYarnContainerId containerId = new AnecYarnContainerId(acquiredWorkerContainers.get(i).getId());
  ...
  private void buildOutputLocations() {
    String anecyarnOutputs = envs.get(AnecYarnConstants.Environment.ANECYARN_OUTPUTS.toString());
    if (StringUtils.isBlank(anecyarnOutputs)) {
      return;
    }
    String[] outputs = StringUtils.split(anecyarnOutputs, "|");
    if (outputs != null && outputs.length > 0) {
      for (String output : outputs) {
        String outputPathTuple[] = StringUtils.split(output, "#");
        if (outputPathTuple.length < 2) {
          throw new RuntimeException("Error input path format " + anecyarnOutputs);
        }
  ...
  private void buildContainerRequest(String[] hostLocals) {
    if (conf.getBoolean(AnecYarnConfiguration.ANECYARN_HOST_LOCAL_ENABLE, AnecYarnConfiguration.DEFAULT_ANECYARN_HOST_LOCAL_ENABLE)) {
      ...
      String hostLocaldir = ayConf.get("fs.defaultFS") + conf.get(AnecYarnConfiguration.ANECYARN_HISTORY_LOG_DIR,
          AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_LOG_DIR) + "/" + conf.get("hadoop.job.ugi").split(",")[0]
          + "/" + envs.get(AnecYarnConstants.Environment.ANECYARN_APP_NAME.toString());
      Path hostLocalPath = new Path(hostLocaldir);
      String line;
      try {
        if (hostLocalPath.getFileSystem(ayConf).exists(hostLocalPath)) {
          FSDataInputStream in = hostLocalPath.getFileSystem(ayConf).open(hostLocalPath);
  ...
  private void buildContainerLocalResource() {
    ...
    containerLocalResource = new HashMap<>();
    try {
      containerLocalResource.put(AnecYarnConstants.ANECYARN_APPLICATION_JAR,
          Utilities.createApplicationResource(appJarRemoteLocation.getFileSystem(conf),
              appJarRemoteLocation,
              LocalResourceType.FILE));
      containerLocalResource.put(AnecYarnConstants.ANECYARN_JOB_CONFIGURATION,
  ...
  private Map<String, String> buildContainerEnv(String role) {
    ...
    containerEnv.put(AnecYarnConstants.Environment.HADOOP_USER_NAME.toString(), conf.get("hadoop.job.ugi").split(",")[0]);
    containerEnv.put(AnecYarnConstants.Environment.ANECYARN_TF_ROLE.toString(), role);
    containerEnv.put(AnecYarnConstants.Environment.ANECYARN_EXEC_CMD.toString(), anecyarnCommand);
    containerEnv.put(AnecYarnConstants.Environment.ANECYARN_APP_TYPE.toString(), anecyarnAppType);
    if (outputIndex >= 0) {
      containerEnv.put(AnecYarnConstants.Environment.ANECYARN_OUTPUTS_WORKER_INDEX.toString(), String.valueOf(outputIndex));
    }
    if (this.inputPath.length() > 0) {
      containerEnv.put(AnecYarnConstants.Environment.ANECYARN_INPUT_PATH.toString(), this.inputPath.substring(0, inputPath.length() - 1));
    }
    if (anecyarnAppType.equals("MXNET") && !single) {
      containerEnv.put(AnecYarnConstants.Environment.ANECYARN_DMLC_WORKER_NUM.toString(), String.valueOf(workerNum));
      containerEnv.put(AnecYarnConstants.Environment.ANECYARN_DMLC_SERVER_NUM.toString(), String.valueOf(psNum));
    ...
    if (anecyarnAppType.equals("DISTLIGHTGBM")) {
      containerEnv.put(AnecYarnConstants.Environment.ANECYARN_LIGHTGBM_WORKER_NUM.toString(), String.valueOf(workerNum));
    }

    if (anecyarnAppType.equals("LIGHTLDA")) {
      containerEnv.put(AnecYarnConstants.Environment.ANECYARN_LIGHTLDA_WORKER_NUM.toString(), String.valueOf(workerNum));
      containerEnv.put(AnecYarnConstants.Environment.ANECYARN_LIGHTLDA_PS_NUM.toString(), String.valueOf(psNum));
    }
    ...
    containerEnv.put(AnecYarnConstants.Environment.APP_ATTEMPTID.toString(), applicationAttemptID.toString());
    containerEnv.put(AnecYarnConstants.Environment.APP_ID.toString(), applicationAttemptID.getApplicationId().toString());

    containerEnv.put(AnecYarnConstants.Environment.APPMASTER_HOST.toString(),
        System.getenv(ApplicationConstants.Environment.NM_HOST.toString()));
    containerEnv.put(AnecYarnConstants.Environment.APPMASTER_PORT.toString(),
        String.valueOf(containerListener.getServerPort()));
    containerEnv.put("PATH", System.getenv("PATH") + ":" + System.getenv(AnecYarnConstants.Environment.USER_PATH.toString()));
    containerEnv.put("LD_LIBRARY_PATH", System.getenv("LD_LIBRARY_PATH") + ":" + System.getenv(AnecYarnConstants.Environment.USER_LD_LIBRARY_PATH.toString()));
    ...
    if (conf.get(AnecYarnConfiguration.ANECYARN_CONTAINER_EXTRAENV) != null) {
      String[] containerUserEnv = StringUtils.split(conf.get(AnecYarnConfiguration.ANECYARN_CONTAINER_EXTRAENV), "|");
  ...
  private List<String> buildContainerLaunchCommand(int containerMemory) {
    ...
    String javaOpts = conf.get(AnecYarnConfiguration.ANECYARN_CONTAINER_EXTRA_JAVA_OPTS, AnecYarnConfiguration.DEFAULT_ANECYARN_CONTAINER_JAVA_OPTS_EXCEPT_MEMORY);
    if (!StringUtils.isBlank(javaOpts)) {
      vargs.add(javaOpts);
    }
    vargs.add(AnecYarnContainer.class.getName());
  ...
  private void launchContainer(Map<String, LocalResource> containerLocalResource,
    ...
    containerEnv.put(AnecYarnConstants.Environment.ANECYARN_TF_INDEX.toString(), String.valueOf(index));
  ...
  private void appendMessage(Message message) {
    if (applicationMessageQueue.size() >= conf.getInt(AnecYarnConfiguration.ANECYARN_MESSAGES_LEN_MAX, AnecYarnConfiguration.DEFAULT_ANECYARN_MESSAGES_LEN_MAX)) {
  ...
  private boolean run() throws IOException, NoSuchAlgorithmException {
    ...
    if (conf.get(AnecYarnConfiguration.ANECYARN_INPUT_STRATEGY, AnecYarnConfiguration.DEFAULT_ANECYARN_INPUT_STRATEGY).equals("STREAM")) {
      buildInputStreamFileStatus();
    } else {
      buildInputFileStatus();
    }

    if ("TENSORFLOW".equals(anecyarnAppType) || "MXNET".equals(anecyarnAppType) || "LIGHTLDA".equals(anecyarnAppType) || "XFLOW".equals(anecyarnAppType)) {
      this.appendMessage("AnecYarn application needs " + workerNum + " worker and "
          + psNum + " ps  containers in fact", true);
    } else {
      this.appendMessage("AnecYarn application needs " + workerNum + " worker container in fact", true);
    }
    ...
    int allocateInterval = conf.getInt(AnecYarnConfiguration.ANECYARN_ALLOCATE_INTERVAL, AnecYarnConfiguration.DEFAULT_ANECYARN_ALLOCATE_INTERVAL);
    ...
    if (anecyarnAppType.equals("MXNET") && !single) {
      ...
      schedulerEnv.add(AnecYarnConstants.Environment.ANECYARN_DMLC_WORKER_NUM.toString() + "=" + workerNum);
      schedulerEnv.add(AnecYarnConstants.Environment.ANECYARN_DMLC_SERVER_NUM.toString() + "=" + psNum);
      schedulerEnv.add("PYTHONUNBUFFERED=1");

      LOG.info("Executing command:" + anecyarnCommand);
      LOG.info("DMLC_PS_ROOT_URI is " + dmlcPsRootUri);
      LOG.info("DMLC_PS_ROOT_PORT is " + dmlcPsRootPort);
      LOG.info(AnecYarnConstants.Environment.ANECYARN_DMLC_WORKER_NUM.toString() + "=" + workerNum);
      LOG.info(AnecYarnConstants.Environment.ANECYARN_DMLC_SERVER_NUM.toString() + "=" + psNum);

      try {
        Runtime rt = Runtime.getRuntime();
        schedulerReservedSocket.close();
        final Process mxnetSchedulerProcess = rt.exec(anecyarnCommand, schedulerEnv.toArray(new String[schedulerEnv.size()]));
    ...
    if (("XFLOW").equals(anecyarnAppType)) {
      ...
      try {
        Runtime rt = Runtime.getRuntime();
        schedulerReservedSocket.close();
        final Process xflowSchedulerProcess = rt.exec(anecyarnCommand, schedulerEnv.toArray(new String[schedulerEnv.size()]));
    ...
    Map<String, String> workerContainerEnv = buildContainerEnv(AnecYarnConstants.WORKER);
    Map<String, String> psContainerEnv = buildContainerEnv(AnecYarnConstants.PS);
    ...
    for (Container container : acquiredPsContainers) {
      ...
      containerListener.registerContainer(new AnecYarnContainerId(container.getId()), AnecYarnConstants.PS);
    }
    index = 0;
    for (Container container : acquiredWorkerContainers) {
      ...
      containerListener.registerContainer(new AnecYarnContainerId(container.getId()), AnecYarnConstants.WORKER);
      if (conf.getBoolean(AnecYarnConfiguration.ANECYARN_TF_EVALUATOR, AnecYarnConfiguration.DEFAULT_ANECYARN_TF_EVALUATOR) && index == workerNum) {
          ...
          while (true) {
            try {
              ...
              while (startSaved) {
                ...
                Utilities.sleep(conf.getInt(AnecYarnConfiguration.ANECYARN_CONTAINER_HEARTBEAT_INTERVAL, AnecYarnConfiguration.DEFAULT_ANECYARN_CONTAINER_HEARTBEAT_INTERVAL));
              }
            } catch (Exception e) {
    ...
    try {
      ...
      Map<AnecYarnContainerId, AnecYarnContainerStatus> lastWorkerContainerStatus = new ConcurrentHashMap<>();
      Map<AnecYarnContainerId, AnecYarnContainerStatus> lastPsContainerStatus = new ConcurrentHashMap<>();
      while (!containerListener.isTrainCompleted()) {
        //report progress to client
        if (conf.getBoolean(AnecYarnConfiguration.ANECYARN_REPORT_CONTAINER_STATUS, AnecYarnConfiguration.DEFAULT_ANECYARN_REPORT_CONTAINER_STATUS)) {
          ...
          for (Container container : workerContainersStatus) {
            if (!lastWorkerContainerStatus.containsKey(new AnecYarnContainerId(container.getId()))) {
              lastWorkerContainerStatus.put(new AnecYarnContainerId(container.getId()), AnecYarnContainerStatus.STARTED);
            }
            if (!applicationContext.getContainerStatus(new AnecYarnContainerId(container.getId())).equals(lastWorkerContainerStatus.get(new AnecYarnContainerId(container.getId())))) {
              this.appendMessage("container " + container.getId().toString() + " status is " + applicationContext.getContainerStatus(new AnecYarnContainerId(container.getId())), false);
              lastWorkerContainerStatus.put(new AnecYarnContainerId(container.getId()), applicationContext.getContainerStatus(new AnecYarnContainerId(container.getId())));
            }
          }
          for (Container container : psContainersStatus) {
            if (!lastPsContainerStatus.containsKey(new AnecYarnContainerId(container.getId()))) {
              lastPsContainerStatus.put(new AnecYarnContainerId(container.getId()), AnecYarnContainerStatus.STARTED);
            }
            if (!applicationContext.getContainerStatus(new AnecYarnContainerId(container.getId())).equals(lastPsContainerStatus.get(new AnecYarnContainerId(container.getId())))) {
              this.appendMessage("container " + container.getId().toString() + " status is " + applicationContext.getContainerStatus(new AnecYarnContainerId(container.getId())), false);
              lastPsContainerStatus.put(new AnecYarnContainerId(container.getId()), applicationContext.getContainerStatus(new AnecYarnContainerId(container.getId())));
            }
          }
        }
        List<Container> workerContainers = applicationContext.getWorkerContainers();
        Map<AnecYarnContainerId, String> clientProgress = applicationContext.getReporterProgress();
        float total = 0.0f;
        for (Container container : workerContainers) {
          String progressLog = clientProgress.get(new AnecYarnContainerId(container.getId()));
      ...
      if (finalSuccess) {
        if ((conf.get(AnecYarnConfiguration.ANECYARN_OUTPUT_STRATEGY, AnecYarnConfiguration.DEFAULT_ANECYARN_OUTPUT_STRATEGY).equals("STREAM")) && outputInfos.size() > 0) {
          LOG.info("ANECYARN_OUTPUT_STRATEGY is STREAM, AM handling the final result...");
          FileSystem fs = new Path(outputInfos.get(0).getDfsLocation()).getFileSystem(conf);
          Map<AnecYarnContainerId, String> mapPath = applicationContext.getMapedTaskID();
          for (Container finishedContainer : acquiredWorkerContainers) {
            String taskID = mapPath.get(new AnecYarnContainerId(finishedContainer.getId()));
          ...
          for (OutputInfo outputInfo : outputInfos) {
            ...
            if (outputIndex >= 0) {
              ...
              if (psNum > 0 && (anecyarnAppType.equals("TENSORFLOW") || anecyarnAppType.equals("LIGHTLDA"))) {
    ...
    int appAttempts = conf.getInt(AnecYarnConfiguration.ANECYARN_APP_MAX_ATTEMPTS, AnecYarnConfiguration.DEFAULT_ANECYARN_APP_MAX_ATTEMPTS);
  ...
  private class RunningAppContext implements ApplicationContext {
    ...
    @Override
    public AnecYarnContainerStatus getContainerStatus(AnecYarnContainerId containerId) {
    ...
    @Override
    public List<InputInfo> getInputs(AnecYarnContainerId containerId) {
    ...
    @Override
    public List<InputSplit> getStreamInputs(AnecYarnContainerId containerId) {
