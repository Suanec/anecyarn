...
public class HsController extends Controller implements AMParams {
  ...
  public void job() throws IOException {
    AnecYarnConfiguration anecyarnConf = new AnecYarnConfiguration();
    jobLogPath = new Path(conf.get(AnecYarnConfiguration.ANECYARN_HISTORY_LOG_DIR,
        AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_LOG_DIR) + "/" + $(APP_ID) + "/" + $(APP_ID));
    LOG.info("jobLogPath:" + jobLogPath);
    String line = null;
    try {
      FSDataInputStream in = jobLogPath.getFileSystem(anecyarnConf).open(jobLogPath);
      ...
      for (String info : readLog.keySet()) {
        if (info.equals(AMParams.APP_TYPE)) {
          if (readLog.get(info) != null) {
            if (readLog.get(info).equals("ANECYARN")) {
          ...
          if (readLog.get(info) instanceof ArrayList<?>) {
            ...
            if (outputList.size() == 0 || outputList.get(0).equals("-")) {
              ...
              for (String output : outputList) {
                set(OUTPUT_PATH + j, output + conf.get(AnecYarnConfiguration.ANECYARN_INTERREAULST_DIR, AnecYarnConfiguration.DEFAULT_ANECYARN_INTERRESULT_DIR));
                j++;
              }
          ...
          if (readLog.get(info) instanceof Map<?, ?>) {
            ...
            if (containerMessage.containsKey(AMParams.CONTAINER_CPU_METRICS)) {
              ...
              if (cpuMetrics != null) {
                ...
                if (map.size() > 0) {
                  cpuMetricsFlag = true;
                  if (containerMessage.get(AMParams.CONTAINER_ROLE).equals(AnecYarnConstants.WORKER) || containerMessage.get(AMParams.CONTAINER_ROLE).equals(AnecYarnConstants.EVALUATOR)) {
