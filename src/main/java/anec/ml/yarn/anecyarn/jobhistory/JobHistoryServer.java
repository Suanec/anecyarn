...
public class JobHistoryServer extends CompositeService {
  ...
  private class HistoryServerSecretManagerService
    ...
    protected void serviceStart() throws Exception {
      boolean recoveryEnabled = getConfig().getBoolean(
          AnecYarnConfiguration.ANECYARN_HS_RECOVERY_ENABLE,
          AnecYarnConfiguration.DEFAULT_ANECYARN_HS_RECOVERY_ENABLE);
      if (recoveryEnabled) {
  ...
  protected void serviceInit(Configuration conf) throws Exception {
    Configuration config = new AnecYarnConfiguration(conf);

    config.setBoolean(Dispatcher.DISPATCHER_EXIT_ON_ERROR_KEY, true);

    // This is required for WebApps to use https if enabled.
    AnecYarnWebAppUtil.initialize(getConfig());
  ...
  protected JHSDelegationTokenSecretManager createJHSSecretManager(
    ...
    long secretKeyInterval =
        conf.getLong(AnecYarnConfiguration.DELEGATION_KEY_UPDATE_INTERVAL_KEY,
            AnecYarnConfiguration.DELEGATION_KEY_UPDATE_INTERVAL_DEFAULT);
    long tokenMaxLifetime =
        conf.getLong(AnecYarnConfiguration.DELEGATION_TOKEN_MAX_LIFETIME_KEY,
            AnecYarnConfiguration.DELEGATION_TOKEN_MAX_LIFETIME_DEFAULT);
    long tokenRenewInterval =
        conf.getLong(AnecYarnConfiguration.DELEGATION_TOKEN_RENEW_INTERVAL_KEY,
            AnecYarnConfiguration.DELEGATION_TOKEN_RENEW_INTERVAL_DEFAULT);
  ...
  protected void doSecureLogin(Configuration conf) throws IOException {
    ...
    SecurityUtil.login(conf, AnecYarnConfiguration.ANECYARN_HISTORY_KEYTAB,
        AnecYarnConfiguration.ANECYARN_HISTORY_PRINCIPAL, socAddr.getHostName());
  }
  ...
  public static InetSocketAddress getBindAddress(Configuration conf) {
    return conf.getSocketAddr(AnecYarnConfiguration.ANECYARN_HISTORY_ADDRESS,
        conf.get(AnecYarnConfiguration.ANECYARN_HISTORY_ADDRESS, AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_ADDRESS),
        conf.getInt(AnecYarnConfiguration.ANECYARN_HISTORY_PORT, AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_PORT));
  }
  ...
  private class deleteLogMonitor implements Runnable {
    ...
    public void run() {
      FileSystem fs;
      Configuration conf = new AnecYarnConfiguration();
      Path historyLog = new Path(conf.get(AnecYarnConfiguration.ANECYARN_HISTORY_LOG_DIR,
          AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_LOG_DIR));
      Path eventLog = new Path(conf.get(AnecYarnConfiguration.ANECYARN_TF_BOARD_HISTORY_DIR,
          AnecYarnConfiguration.DEFAULT_ANECYARN_TF_BOARD_HISTORY_DIR));
      int monitorInterval = conf.getInt(AnecYarnConfiguration.ANECYARN_HISTORY_LOG_DELETE_MONITOR_TIME_INTERVAL,
          AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_LOG_DELETE_MONITOR_TIME_INTERVAL);
      int logMaxAge = conf.getInt(AnecYarnConfiguration.ANECYARN_HISTORY_LOG_MAX_AGE_MS,
          AnecYarnConfiguration.DEFAULT_ANECYARN_HISTORY_LOG_MAX_AGE_MS);
